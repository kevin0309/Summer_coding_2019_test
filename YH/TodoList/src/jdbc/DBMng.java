 ...
 * 		dbMng.setString(1, {지정해야 할 파라미터가 있을 시 파라미터 값});<br>
 * 		dbMng.execute();<br><br>
 * 
...
public class DBMng {
	...
	public DBMng() throws SQLException {
		...
		///curent project setting
		try {
			setConnection(ConnectionProvider.getConnection());
		} catch (ConnectDBException e) {
			addError("get connection error");
	 ...
	 * @throws SQLException
	 */
	public void setString_Xss(int index, String param) throws SQLException {
		if (checkParams(index) && checkSTATUS(DBMngStatus.STATEMENT_READY)) {
			pstmt.setString(index, param);
			pstmtParamCnt.add(true);
			checkStatement();
	 ...
	 * @throws SQLException
	 */
	public void setString_Xss(String param) throws SQLException {
		int index = pstmtParamCnt.size()+1;
		if (checkParams(index) && checkSTATUS(DBMngStatus.STATEMENT_READY)) {
			pstmt.setString(index, param);
			pstmtParamCnt.add(true);
